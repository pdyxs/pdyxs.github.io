...
describe('displayFor', () => {
  it('returns the declared display info when the value is in the map', () => {
    const display = { 'what:puzzles': { name: 'Puzzles', description: 'Logic puzzles' } };
    expect(displayFor('what:puzzles', display)).toEqual({ name: 'Puzzles', description: 'Logic puzzles' });
  });

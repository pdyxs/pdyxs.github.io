...
type StoryEntry = { id: string; data: { series: string; order: number; published?: boolean; title?: string } };
...
describe('getSeriesSiblings', () => {
  it('getSeriesSiblings_first_chapter: order=0 entry → prev undefined, next is order=1, current=1', () => {
    ...
    const result = getSeriesSiblings(ch0, [ch0, ch1, ch2], false);
  ...
  it('getSeriesSiblings_last_chapter: last entry → prev is penultimate, next undefined', () => {
    ...
    const ch1 = fakeStory({ id: 'arctic/ch-1', order: 1 });
    const ch2 = fakeStory({ id: 'arctic/ch-2', order: 2 });
    const result = getSeriesSiblings(ch2, [ch0, ch1, ch2], false);
  ...
  it('getSeriesSiblings_mid_chapter: middle entry → both prev and next defined, current and total correct', () => {
    ...
    const ch1 = fakeStory({ id: 'arctic/ch-1', order: 1 });
    const ch2 = fakeStory({ id: 'arctic/ch-2', order: 2 });
    const result = getSeriesSiblings(ch1, [ch0, ch1, ch2], false);
  ...
  it('getSeriesSiblings_prod_excludes_unpublished: published:false with isDev=false → excluded from siblings and count', () => {
    const ch0 = fakeStory({ id: 'arctic/ch-0', order: 0 });
    const ch1 = fakeStory({ id: 'arctic/ch-1', order: 1, published: false });
    const ch2 = fakeStory({ id: 'arctic/ch-2', order: 2 });
    const result = getSeriesSiblings(ch0, [ch0, ch1, ch2], false);
  ...
  it('getSeriesSiblings_dev_includes_unpublished: published:false with isDev=true → included normally', () => {
    const ch0 = fakeStory({ id: 'arctic/ch-0', order: 0 });
    const ch1 = fakeStory({ id: 'arctic/ch-1', order: 1, published: false });
    const ch2 = fakeStory({ id: 'arctic/ch-2', order: 2 });
    const result = getSeriesSiblings(ch0, [ch0, ch1, ch2], true);
    expect(result.total).toBe(3);
    expect(result.next).toBe(ch1);

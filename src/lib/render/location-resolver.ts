...
import { COLLECTION_RENDERERS, NAV_RENDERERS, COLLECTION_VIEW_RENDERERS, GENERIC_RENDERERS } from '@render/renderers';
import GenericRenderer from '@components/card-renderers/GenericRenderer.astro';
import { getLensDefinition } from '@browse/lenses/lens-registry';
...
export function resolveCardRenderer(rendererName: string): AstroComponentFactory {
  ...
  throw new Error(
    `renderer: "${rendererName}" is neither registered nor declared generic. ` +
      ...
      `(src/lib/render/renderers.ts) if GenericRenderer already expresses it.`,
  );
}
  ...
  if (!registered) {
    throw new Error(
      `navRenderer: "${navRendererName}" is not registered. ` +
        ...
        `Add it to NAV_RENDERERS in src/lib/render/renderers.ts, or remove the declaration.`,
    );
  }
  return registered;
}

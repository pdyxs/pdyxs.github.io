...
import { parseEmbedUrl, embedPosterUrl } from '@render/embeds.ts';
import { isVaultInfrastructurePath } from '@content/folders/content-glob.ts';
 ...
 * reference resolves (see auditCards in src/lib/site/audit.ts).
 */
 ...
 * `npm run pad:images` (see src/lib/render/image-padding.ts). Left in, the gallery

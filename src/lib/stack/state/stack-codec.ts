...
// manifest, src/lib/stack/state/stack-manifest.ts) followed by zero or more `~`-separated
// param tokens; entries within from/to are `.`-separated. Each param token is
// produced by the codec registry (src/lib/stack/state/param-codecs.ts) — filters resolve
...
import { allocateSlot, cardEntry, lensEntry, withFreeSlot } from '@stack/layout/stack-layout';
import { filtersForKey, isLensUid, lensNameForKey, splitLocationParams } from '@browse/lenses/lens-key';
import type { ManifestLookup } from '@stack/state/stack-manifest';
import { encodeParam, decodeParam } from '@stack/state/param-codecs';
import type { CodecContext } from '@stack/state/param-codecs';

...
//
//   - `apply(tags, card)` runs per-card in getAllCards() (src/lib/content/cards/cards.ts) and
...
} from '../cards/difficulty.ts';
import type { Action } from '../cards/card-actions.ts';
...
export type FilterGeneratorCard = {
   ...
   * getAllCards() (src/lib/content/cards/cards.ts) as `frontmatter[key] ?? cascade[key]`.
   *

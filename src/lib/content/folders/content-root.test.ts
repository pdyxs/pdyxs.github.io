...
import { CONTENT_ROOT, assertContentRoot } from '@content/folders/content-root';
import { makeFileReader, resolveFolderCascade } from '@content/folders/folder-config';
import { discoverTagSources, makeContentTreeReader } from '@content/tags/tag-registry';
...
describe('content root resolution', () => {
  ...
  it('neither reader derives its path from its own module location', async () => {
    ...
    for (const file of [
      'content/folders/folder-config.ts',
      'content/tags/tag-registry.ts',
      'content/folders/content-root.ts',
    ]) {
  ...
  it('fails loudly rather than silently when run outside the project', async () => {
    ...
    const entry = resolve(PROJECT_ROOT, 'src/lib/content/folders/content-root.ts');
    await expect(
      execFileAsync(
        process.execPath,

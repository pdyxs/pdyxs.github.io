...
import { resolveCardTitle } from '../../stack/state/card-title';
export { resolveCardTitle };
import { derivePathTags, mergeEffectiveTags } from '../tags/tag-inheritance';
import { resolveFolderCascade, makeFileReader } from '../folders/folder-config';
import type { FolderCascade } from '../folders/folder-config';
import {
  generateTagsForCard,
  generatorDerivations,
  generatorFrontmatterKeys,
} from '../tags/filter-generators';
import { applyReEnables, parseExcludeTags, partitionGeneratedTags } from '../tags/exclude-tags';
import { resolveActions, type ActionSource } from './card-actions';
import { computeAffiliationTags } from '../tags/affiliations';
import { discoverAffiliations, discoverTagPriorities } from '../tags/tag-registry';
import { interpolate } from '../folders/interpolate';
...
import { DEFAULT_FOLDER_SORT, resolveSortValue, type FolderSort } from '../folders/folder-sort';
import { withUninspectedTag } from '../tags/uninspected-facet';
...
export type CardFrontmatter = {
   ...
   * src/lib/content/tags/uninspected-facet.ts for the dev-only `why:uninspected` filter it
   * drives, and src/lib/site/audit.ts for the `not-inspected` finding. */
  inspected?: boolean;
  titleSuffix?: string;
  width?: string;
  dateLabel?: string;

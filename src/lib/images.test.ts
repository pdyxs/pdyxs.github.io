...
describe('resolveGalleryImages', () => {
  it('defaults to colocated images excluding the header image', () => {
    const result = resolveGalleryImages('what/projects/art-heist', 'outside.jpg', []);
    expect(result).toEqual([]);
  });

  it('defaults to colocated images when no header image is set', () => {
    const result = resolveGalleryImages('what/projects/art-heist', undefined, []);
    expect(result).toHaveLength(1);
    expect(typeof result[0].src).toBe('object');
  });

  it('uses the images[] override when present, resolving bare filenames', () => {
    const result = resolveGalleryImages('what/projects/art-heist', 'outside.jpg', ['outside.jpg']);
    expect(result).toHaveLength(1);
    expect(typeof result[0].src).toBe('object');
  });

  it('passes remote image URLs from images[] through as-is', () => {
    const result = resolveGalleryImages('what/projects/art-heist', undefined, ['https://example.com/x.jpg']);
  ...
  it('drops images[] entries with no colocated file', () => {
    const result = resolveGalleryImages('what/projects/art-heist', 'outside.jpg', ['missing.jpg']);
    expect(result).toEqual([]);
  });

<script lang="ts">
  ...
  import type { LensDefinition } from '@browse/lenses/lens-registry';
  import { lensUid } from '@browse/lenses/lens-registry';
  import LensIcon from '@components/filters/LensIcon.svelte';
  ...
  interface Props {
    ...
    onDrillBack: () => void;
    onClear: () => void;
    /** Lenses filed under this dimension (src/lib/browse/lenses/lens-registry.ts) — listed
  ...
  // the ahead fan's crop — see src/styles/stack-desktop.css § THE ASYMMETRY).
  // `position: fixed`
...
<style>
  ...
  /* Full-bleed rows: every row's highlight (hover and selected alike) is the
     ...
     global `li { margin-bottom }` (src/styles/base.css) and the list paddings are the
  ...
  /* Active lens reads selected, mirroring an active tag
     (.browse-dim-item--selected). See --color-selected-* in src/styles/tokens.css. */

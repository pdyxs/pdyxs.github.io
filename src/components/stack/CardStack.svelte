<script lang="ts">
  ...
  import { scrollBehaviourFor, scrollSettleAction, transitionWillFire, widthTransitionOf, SCROLL_SETTLE_TIMEOUT_MS, STACK_RESIZING_ATTR } from '@stack/layout/stack-motion';
  import { filtersForKey, isLensUid, lensNameForKey, splitLocationParams } from '@browse/lenses/lens-key';
  import { lensFilterStore } from '@stores/lens-filter-store';
  import { filterStateFromParams } from '@dimensions';
  import { serialiseStack, deserialiseStack, locationParamsFromSearch } from '@stack/state/stack-codec';
  import type { ParamPairs } from '@stack/state/stack-codec';
  import { paramsAfterSlotReplace } from '@stack/state/slot-params';
  import { manifestLookup } from '@stack/state/stack-manifest-client';
  import { tagManifestLookup } from '@content/tags/tag-manifest-client';
  import { parseCollectionLink } from '@render/collection-link';
  import { stackFromParams } from '@stack/state/browse-stack';
  import { filterUrlForTagValue } from '@dimensions';
  import { markRead, readToRecord } from '@stack/state/card-view-state';
  import { placeholderTitle } from '@stack/state/card-title';
  import { lensChromeForKey } from '@browse/lenses/lens-chrome';
  import { waitForTransition } from '@stack/layout/transition-wait';
  import StackFragment from '@components/stack/StackFragment.svelte';
  import {
    createCardFragments,
    titleOfElement,
    uidToFetchUrl,
    urlToUid,
  } from '@stack/state/card-fragments';
  ...
  // width both derive from the same var — see src/styles/stack-desktop.css).
  // No declared width
  ...
  // peek. The arithmetic is `scrollTargetFor` (src/lib/stack/layout/stack-geometry.ts,
   ...
   * `src/styles/tokens.css` — the same knob every stack duration is a `calc()` of.
   *
  ...
  /** Safety net for the closing card's collapse, which `src/styles/stack.css`
   * declares at

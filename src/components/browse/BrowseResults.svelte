<script lang="ts">
  ...
  import type { StripTerminal } from '@browse/lenses/strip-lens';
  import {
    revealedAfter,
    revealButtonLabel,
    revealSettings,
    ...
    REVEAL_ROOT_MARGIN,
    type RevealSettings,
  } from '@browse/results/progressive-reveal';
  ...
  interface Props {
     ...
     * reason (see historyEmptyMessage in src/lib/browse/lenses/history-lens.ts).
     */
    emptyMessage?: string;
    /**
...
<main class="fp-browse-grid" aria-label="Browse results">
  ...
  <!-- The assembly-resize placeholder (issue #126; extracted in #149). This
       ...
       rules in src/styles/tokens.css, the only thing that ever reveals it. (The

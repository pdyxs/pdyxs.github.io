<script lang="ts">
  ...
  import { computeStatusBadge } from '@render/status-badge';
  // Shared with GenericRenderer's dateline so a card's date reads identically
  // in a listing and on the card itself — see lib/content/cards/card-date.ts.
  import { formatCardDate } from '@content/cards/card-date';
  import { resolveBrowseCardVariant } from '@browse/results/browse-card-variants';
  import type { BrowseCardVariantName } from '@browse/results/browse-card-variants';
